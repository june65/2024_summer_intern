...
def save_gif(_path, predictions, ground_truths, seqs_used, pedestrian_used):
    ...
    pred_lines = {id_: ax.plot([], [], linestyle='dotted', color=id_color_map[id_])[0] for id_ in ids}
    gt_lines = {id_: ax.plot([], [], linestyle='-', color=darken_color(id_color_map[id_]))[0] for id_ in ids}
    ...
    def update(frame):
        ...
        for id_ in pedestrian_used[frame]:
            ...
            if len(ped_data) > 0:
                ...
                if frame > 8:
                    if id_ in pedestrian_used[frame - 8] :
                        pred_in = predictions[frame - 8][:,ped_id_seq_num[id_][frame - 8]]
                        gt_in = ground_truths[frame - 8][:,ped_id_seq_num[id_][frame - 8]]
                    ...
                    if len(pred_history[id_]) > 0:
                        pred_history[id_].pop(0)
                        gt_history[id_].pop(0)
                    pred_lines[id_].set_alpha(0.5)
                    gt_lines[id_].set_alpha(0.5)
    ...
    gif_path = _path.replace('.txt', '.mp4').replace('/datasets/', '/visual_PHYSIOGCN/')
    os.makedirs(os.path.dirname(gif_path), exist_ok=True)
    ani.save(gif_path, writer=animation.FFMpegWriter(bitrate=10000))
    #plt.show()
    
def load_model_with_flexible_state_dict(model, model_path):
    # Load the state dict from the checkpoint
    checkpoint = torch.load(model_path)
    model_state_dict = model.state_dict()

    # Load the state dict in a flexible manner
    ...
    for k, v in pretrained_state_dict.items():
        if k in model_state_dict:
            if model_state_dict[k].size() == v.size():
                ...
                print(f"Size mismatch for {k}: checkpoint size {v.size()} vs model size {model_state_dict[k].size()}")
        else:
    ...
    for root, dirs, files in os.walk('./datasets/'):
        ...
        for filename in files:
            if filename.endswith('.txt'):
                ...
                dataset_ = root.split('/')[2].split("\\")[0]
                dataset_path = './datasets/' + dataset_ + '/'
                checkpoint_dir = './checkpoint/physiogcn_'+ dataset_ +'_experiment/'
                args_path = checkpoint_dir + '/args.pkl'
                with open(args_path, 'rb') as f:
                    args = pickle.load(f)
                model_path = checkpoint_dir + dataset_ + '_best.pth'

                test_dataset = Dataload(file_path, obs_len=args.obs_seq_len, pred_len=args.pred_seq_len, skip=1)
                ...
                model = model.cuda()
                model = load_model_with_flexible_state_dict(model, model_path)
                ...
                for batch in test_loader:
                    ...
                    _, _, V_refi, _ = model(S_obs, pruning=4, clustering=True)
                    V_refi_new =torch.mean(V_refi, dim=0)
                    all_predictions.append(V_refi_new.cpu().detach().numpy())
                    all_ground_truths.append(S_trgt[:, 0].squeeze(dim=0).cpu().detach().numpy())
                ...
                save_gif(file_path, sorted_predictions, sorted_ground_truths, test_dataset.seqs_used, test_dataset.pedestrian_ids)
